...
# ==========================
# 2. CHỌN THUẬT TOÁN TRÊN SIDEBAR
# ==========================
...
model, df, numeric_features, categorical_features = train_model(model_type)

# ==========================
# 3. GIAO DIỆN NHẬP THÔNG TIN
# ==========================

st.title(f"Demo dự đoán mức độ béo phì (Dữ liệu Châu Á - {model_type})")
...
for col in categorical_features:
    # ĐỔI NHÃN HIỂN THỊ CHO CỘT "Tần suất ăn giữa các bữa"
    label = col
    if col == "Tần suất ăn giữa các bữa":
        label = "Tần suất ăn các bữa phụ"

    options = sorted(df[col].dropna().unique())

    # THÊM "xe máy" VÀO CỘT "Phương tiện di chuyển"
    if col == "Phương tiện di chuyển":
        options = list(options)
        if "xe máy" not in options and "Xe máy" not in options:
            options.append("xe máy")

    value = st.selectbox(label, options, index=0)
    nguoi_moi[col] = value
